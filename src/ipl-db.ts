...
import { IPL_DB_PATH, IPL_GRACE_PERIOD_HOURS } from "./ipl-config";
...
export function getIplDb(): Db {
  if (_db) return _db;

  _db = new DatabaseSync(IPL_DB_PATH);
  _db.exec(`
    ...
    CREATE INDEX IF NOT EXISTS idx_ipl_notifications_due
...
export function insertNotificationIfMissing(
  ...
  db.prepare(`
    ...
    VALUES (?, ?, ?)
  `).run(matchId, type, scheduledAt);
}

export function getDueNotifications(db: Db): DueNotification[] {
  const now = new Date().toISOString();
  const cutoff = new Date(Date.now() - IPL_GRACE_PERIOD_HOURS * 60 * 60 * 1000).toISOString();
